...
int main(){
    ...
    fcl::test::loadOBJFile("/home/ubuntu/fcl/my/rob.obj", vertices, triangles);
    ...
    while(true){
        int i;
        std::cin>>i;
        std::cout<<"point: ["<<posVec[i][0]<<", "<<posVec[i][1]<<", "<<posVec[i][2]<<"]"<<std::endl;
        std::cout<<"triangle index: ["<<triVec[i][0]<<", "<<triVec[i][1]<<", "<<triVec[i][2]<<"]"<<std::endl;
    }
    ...
    fcl::Vector3d T(1.05, 0.05, 0);
    fcl::Transform3d pose2 = fcl::Transform3d::Identity();
    pose2.translation() = T;
    std::shared_ptr<fcl::Sphered> sphere = std::make_shared<fcl::Sphered>(0.1);
